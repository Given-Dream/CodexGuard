...
- 单文件 Release 启动器必须内嵌完整包、拒绝越界路径和重解析点、逐文件校验释放结果，并且不得删除、移动或覆盖已有缓存；
...
完整发布构建还会生成稳定命名的便携 ZIP 和内嵌该 ZIP 的单文件启动器，并实际运行一次自解包与包内哈希校验：

```powershell
powershell -ExecutionPolicy Bypass -File .\Package.ps1 -Label preview
```

只有原有测试全部通过、单文件启动器成功释放完整载荷，并且包内主程序核对三个必要 EXE 的 SHA-256 成功时，打包才返回成功。启动器不自动删除、移动或覆盖已有缓存；发现同名缓存异常时会使用新的恢复目录并停止信任异常副本。
